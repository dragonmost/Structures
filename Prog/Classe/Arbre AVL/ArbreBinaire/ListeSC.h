...
{
private:
	CCelluleListe<int>* m_pPremier;			//pointeur vers le premier objet de la liste
	CCelluleListe<int>* m_pDernier;			//pointeur vers le dernier objet de la liste
	unsigned int m_uiCompte;					//le nombre d'objet dans la liste

public:
#define LISTNULL -2147483647;				//remplace nullptr
	...
	_Element: le int a mettre dans la liste*/
	void Ajouter(int _Element)
	{
		if (m_pPremier == nullptr)
		{
			m_pPremier = new CCelluleListe<int>(_Element, nullptr);
			m_pDernier = m_pPremier;
		}
		else
		{
			CCelluleListe<int>* pTemp = m_pDernier;
			m_pDernier = new CCelluleListe<int>(_Element, nullptr);
	...
	bool Inserer(int _Element, int _Position)
	{
		if (_Position <= m_uiCompte + 1 && _Position >= 1)
		{

			CCelluleListe<int>* pRunner = m_pPremier;
			if (_Position == 1){
				m_pPremier = new CCelluleListe<int>(_Element, pRunner);
				if (pRunner == nullptr)
					m_pDernier = m_pPremier;
			}
			else{
				...
				pRunner->DefinirSuivant(new CCelluleListe<int>(_Element, pRunner->ObtenirSuivant()));
	...
	retourne un int qui represente l'element dans la liste*/
	int Obtenir(int _Position)
	{
	...
	bool retirer(int _Position)
	{
		...
		if (_Position == 1)
		{
			CCelluleListe<int>* pTemp = m_pPremier;
		...
		if (_Position < m_uiCompte)
		{
			...
			for (int ii = 1; ii < _Position - 1; ii++)
				pRunner = pRunner->ObtenirSuivant();
			CCelluleListe<int>* pTemp = pRunner->ObtenirSuivant();
